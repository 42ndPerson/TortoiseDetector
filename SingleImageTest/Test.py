...
#   Catches even obscure tortoises and synthetic night tortoise
#   Test with synthetic data suggests that night correction would remove its ability to detect tortoises at night
# ViT-g-14 (Checkpoint: laion2b_s34b_b88k)
#   Intermediate model that somehow performs worse than the light model
#   Needs night correction to not false positive on all night samples, but likely looses ability to detect night tortoises
#   Cannot detect synthetic night tortoise with night correction and sometimes fails at night/day differentiation
...
# Model Setup
model_name = 'ViT-g-14'#'PE-Core-bigG-14-448'#'ViT-g-14'#'ViT-B-32'
checkpoint_name = 'laion2b_s34b_b88k'#'meta'#'laion2b_s34b_b88k'#'laion2b_s34b_b79k'
...
# Data
images = [preprocess(Image.open("Desert.png")).unsqueeze(0), preprocess(Image.open("NightDesert.png")).unsqueeze(0), preprocess(Image.open("Tortoise.png")).unsqueeze(0), preprocess(Image.open("ObscureTortoise.png")).unsqueeze(0), preprocess(Image.open("ObscureTortoise2.png")).unsqueeze(0), preprocess(Image.open("GeminiSyntheticNightTortoise.png")).unsqueeze(0)]
